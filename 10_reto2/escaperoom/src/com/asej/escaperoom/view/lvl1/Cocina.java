...
public class Cocina extends JPanel {
	...
	private int MISIONES_OBJETIVO = 4; // encontrar mochila, entregar objetos a padre, madre e hija
	private boolean padrePeticionHecha = false;
	private boolean madrePeticionHecha = false;
	private boolean hijaPeticionHecha = false;
	private boolean objetoPadre = false;
	private boolean objetoMadre = false;
	private boolean objetoHija = false;
	...
	public Cocina(Ventana ventana) {
		...
		/*
		 * btnOpcion3 = new JButton(""); btnOpcion3.setBounds(750,90,300,40);
		 * btnOpcion3.setBackground(Color.ORANGE); mensajes.add(btnOpcion3);
		...
		hija_Button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				hija_Button.setEnabled(false); // Deshabilitar el botón
			}
		});

		btnOpcion1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				switch (mensajePantalla) {
					case "padre" -> {
					...
					case "hija" -> {
						dialogo_textPane.setText("Hija: Hijo, ya que estas aqui, hazme un favor, ve al garaje y traeme mi caja de herramientas porfavor, creo que la deje cerca del coche\n");
						btnOpcion1.setText("¡Claro, ahora mismo!");
						btnOpcion2.setText("No me apetece mucho");
						btnOpcion1.setVisible(true);
						btnOpcion2.setVisible(true);
					...
					default -> {
						System.out.println("fin dialogo");
						mensajes.setVisible(false);
						btnOpcion1.setVisible(false);
						btnOpcion2.setVisible(false);
						padre_Button.setVisible(true);
						madre_Button.setVisible(true);
						hija_Button.setVisible(true);
					}
				}
...
//				});
	...
	public void pasarPreguntaPadre() {
		...
		if (!padrePeticionHecha) {
			...
			dialogo_textPane.setText("Aún estoy esperando las herramientas");

		} else if (padrePeticionHecha && objetoPadre) {
			mensajes.setVisible(true);
			dialogo_textPane.setText(
					"Muchas gracias, cuando vuelvas del colegio, te llevo al cine como recompensa, que seguro ya habre arreglado el coche\n");
		} else {
			mensajes.setVisible(false);
		}
